...
template <typename T>
__fd__ constexpr T& min(T& x, T& y) { return x <= y ? x : y; }

template <typename T>
__fd__ constexpr T& max(T& x, T& y) { return x >= y ? x : y; }
...
template <typename T>
__device__ constexpr T constexpr_gcd(T u, T v)
{
	return (v == 0) ? u : gcd(v, u % v);
}

template <typename T>
__device__ constexpr T constexpr_lcm(T u, T v)
{
...
namespace detail {
template <typename T>
__device__ constexpr T constexpr_sqrt_helper(T x, T low, T high)
{
...
template <typename T>
__device__ constexpr T constexpr_sqrt(T& x)
{
