...
{
	T tmp = value;
	for(promoted_size<Size> index = thread::index();
		index < count;
		index += grid_info::linear::block::size())
	{
		start[index] = tmp;
	}
}
...
inline __device__ void fill(ForwardIterator start, ForwardIterator end, const T& value)
{
    ...
    for (; iter < end; iter += grid_info::linear::block::length())
    {
    	*iter = tmp;
    }
}
...
inline __device__ void memzero(ForwardIterator start, Size count)
{
    return fill_n(start, count, 0);
}

template <typename T>
inline __device__ void fill_shared_memory(const T& value, shared_memory_size_t count)
{
	T tmp = value;
	fill_n(shared_memory_proxy<T>(), value, count);
}

template <typename T>
inline __device__ void fill_shared_memory(const T& value)
{
	T tmp = value;
	auto count = ptx::special_registers::dynamic_smem_size() / sizeof(T);
	fill_n(shared_memory_proxy<T>(), value, count);
}
	...
	Size                    length)
{
	using namespace grid_info::linear;
	#pragma unroll
	for(promoted_size<Size> pos = thread::index_in_block(); pos < length; pos += grid_info::linear::block::size()) {
...
/**
 * Sets the (beginning of the) shared memory of the block
 ...
 */
template <typename T, typename Size>
__fd__ T* __restrict__ set_shared_memory_to(
	const T*  __restrict__  source,
	Size                    length)
{
	T* __restrict__ data_in_shared_mem = shared_memory_proxy<T>();
	...
	Size                   length)
{
	...
	for(int pos = thread::index(); pos < length; pos += grid_info::linear::block::size()) {
...
forceinline __device__ void elementwise_apply(
	...
	Size                           length)
{
	using namespace grid_info::linear;
	Operation op;
	for(promoted_size<Size> pos = thread::index(); pos < length; pos += grid_info::linear::block::size()) {
...
{
	#pragma unroll
	for(promoted_size<Size> pos = thread::index_in_block();
		pos < length;
		pos += ::grid_info::linear::block::length())
	{
		f(pos);
	}
}
...
{
	if (grid_info::linear::lane::index() == writing_lane_index) {
		where_to_make_available[grid_info::linear::warp::index()] = datum;
	}
 ...
 __inline__ __device__ typename ReductionOp::result_type reduce(InputDatum value)
{
	...
	result_type intra_warp_result = warp::reduce<ReductionOp>(static_cast<result_type>(value));
	...
	if (!AllThreadsObtainResult) {
		...
		if (!grid_info::linear::warp::is_first_in_block()) { return op.neutral_value(); }
	}
	...
	result_type other_warp_result  =
		(lane::index() < grid_info::linear::block::num_warps()) ?
		warp_reductions[lane::index()] : op.neutral_value();

	return warp::reduce<ReductionOp>(other_warp_result);
}
	 ...
	 InputDatum value)
{
	...
	result_type intra_warp_inclusive_scan_result =
		warp::scan<ReductionOp, InputDatum, inclusivity_t::Inclusive>(static_cast<result_type>(value));

	primitives::block::share_warp_datum_with_whole_block(
		intra_warp_inclusive_scan_result, scratch, grid_info::linear::warp::last_lane);
	...
	if (grid_info::linear::warp::is_first_in_block()) {
		...
		auto warp_reductions_scan_result =
			warp::scan<ReductionOp, result_type, inclusivity_t::Exclusive>(
				scratch[lane::index()]);
	...
	auto inclusive_reduction_result_upto_previous_warp = scratch[grid_info::linear::warp::index()];
	...
	if (Inclusivity == inclusivity_t::Inclusive) {
		...
		result_type shuffled = __shfl_up(intra_warp_inclusive_scan_result, 1);
	 ...
	 typename ReductionOp::result_type&              reduction_result)
{
	...
	result_type intra_warp_inclusive_scan_result =
		warp::scan<ReductionOp, InputDatum, inclusivity_t::Inclusive>(
	...
	auto num_warps = grid_info::linear::block::num_warps();
	...
	if (grid_info::linear::warp::is_first_in_block()) {
		...
		auto warp_reductions_scan_result =
			warp::scan<ReductionOp, result_type, inclusivity_t::Exclusive>(
				scratch[grid_info::linear::lane::index()]);
		scratch[grid_info::linear::lane::index()] = warp_reductions_scan_result;
	}
	__syncthreads();
...
#define once_per_block if (::grid_info::linear::thread::is_first_in_block())
...
__fd__ T get_from_thread(const T& value, unsigned source_thread_index)
{
	...
	if (grid_info::linear::thread::index_in_block() == source_thread_index) {
		tmp = value;
	}
	__syncthreads();
	return tmp;
